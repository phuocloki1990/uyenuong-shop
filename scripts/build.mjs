...
export function build(root = process.cwd(), { check = false } = {}) {
  ...
  function related(x) {
    ...
    for (const kind of ['products', 'articles']) {
      ...
      if (items.length) {
        sections.push(
          `<section class="section-sm"><h2>${kind === 'products' ? 'Sản phẩm liên quan' : 'Bài viết liên quan'}</h2><div class="${kind === 'products' ? 'featured-grid' : 'guides-grid'}">${items.slice(0, 6).map(kind === 'products' ? productCard : articleCard).join('')}</div></section>`
        );
      }
    }
...
if (
  ...
  try {
    ...
    console.log(
      `${flags.includes('--check') ? 'Kiểm tra đạt' : 'Đã build'}: ${result.products} sản phẩm, ${result.articles} bài viết, ${result.categories} chuyên mục; ${result.redirects} redirect; xóa ${result.removed.length} trang generated cũ.`
    );
  } catch (error) {
    console.error(
      'BUILD DỪNG:',
